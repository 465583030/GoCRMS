...
//
// Usage: gocrmsd <name_prefix> <parellel_count> <endpoint> <server_count>
// example:
// $ bsub -R "type==any" -q pwodebug "gocrmsd sb 100 172.20.0.18:12379 500"
// $ etcdctl --endpoints 172.20.0.18:12379 get --prefix crms/server | grep 100 | wc -l
...
func main() {
	...
	nServer, err := strconv.Atoi(flag.Arg(3))
	if err != nil {
		log.Fatal(err)
	}

	servers := make([]*server.Server, nServer)
	for i := 0; i < nServer; i++ {
		// connect and create server
		serverName := fmt.Sprintf("%s_%04d", name, i)
		server, err := server.NewServer(serverName, parellelCount, endpoints, dialTimeout, requestTimeout)
		if err != nil {
			log.Fatal(err)
		}
